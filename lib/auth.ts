...
export async function validateApiKey(
  ...
  if (currentUsage >= limit) {
    return {
      valid: false,
      error: `Rate limit exceeded. ${keyData.tier} tier allows ${limit} requests/month. Current: ${currentUsage}. Upgrade at https://govcon.ezbizservices.com/pricing`,
    };
  }
...
export async function createApiKey(
  ...
  const store = await loadKeys();
  const key = `sk_gov_${randomBytes(24).toString("hex")}`;
  store.keys[key] = {
    name,
    email,
    tier,
